...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("WWcli.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("WWCLI_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".WWcli", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("WWcli-wechat.pid");
    }
}
