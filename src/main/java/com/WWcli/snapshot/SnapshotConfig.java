...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("WWcli.snapshot.enabled", "WWCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("WWcli.snapshot.dir", "WWCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".WWcli", "snapshots").toString()));
        int max = readInt("WWcli.snapshot.max", "WWCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("WWcli.snapshot.excludes", "WWCLI_SNAPSHOT_EXCLUDES", ""));
