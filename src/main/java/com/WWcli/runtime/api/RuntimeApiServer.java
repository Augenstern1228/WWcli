package com.WWcli.runtime.api;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.WWcli.runtime.task.TaskRunner;
...
public class RuntimeApiServer implements AutoCloseable {
    ...
    private final ExecutorService executor = Executors.newCachedThreadPool(r -> {
        Thread thread = new Thread(r, "WWcli-runtime-api");
    ...
    public RuntimeApiServer(RuntimeThreadStore store, TaskRunner runner, int port, String apiKey) throws IOException {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalArgumentException("Runtime API 需要配置 WWCLI_RUNTIME_API_KEY 或 -DWWcli.runtime.api.key");
        }
        this.store = store;
        this.runner = runner;
        this.apiKey = apiKey;
    ...
    public static String configuredApiKey() {
        String configured = System.getProperty("WWcli.runtime.api.key");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("WWCLI_RUNTIME_API_KEY");
        }
        return configured;
    }
    ...
    private boolean authorized(HttpExchange exchange) {
        ...
        String direct = exchange.getRequestHeaders().getFirst("X-WWcli-API-Key");
