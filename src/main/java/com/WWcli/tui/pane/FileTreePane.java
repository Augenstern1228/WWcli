...
public class FileTreePane extends Panel {
     ...
     * @param config 配置
     */
    public FileTreePane(WWcliConfig config) {
        super();
     ...
     * 加载忽略规则（用户级 + 内置默认）。
     */
    private static List<String> loadIgnorePatterns(WWcliConfig config) {
        List<String> patterns = new ArrayList<>(DEFAULT_IGNORE);

        // 读取用户级 filetree-ignore.txt
        Path userIgnore = Path.of(System.getProperty("user.home"), ".WWcli", "filetree-ignore.txt");
