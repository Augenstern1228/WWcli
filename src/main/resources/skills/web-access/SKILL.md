...
- `close_page` 只能关 WWcli 自己 `new_page` 出来的 tab，不要尝试关用户原有的 Gmail / Slack。
...
```bash
ls ~/.WWcli/skills-cache/web-access/references/site-patterns/
```
...
**写回机制**：操作中如果发现新站点的陷阱或有效模式，主动写到 `~/.WWcli/skills/web-access/references/site-patterns/<domain>.md`（用户级目录，不要写 jar 内置缓存），格式参考已有文件。
...
- 不要替用户输用户名密码：WWcli 不做自动登录
- 不要把 references/ 写回 jar 缓存目录：写到 `~/.WWcli/skills/web-access/references/site-patterns/` 用户级目录
