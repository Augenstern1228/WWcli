 ...
 * 1. 系统属性：{@code WWcli.react.token.budget} / {@code WWcli.react.stagnation.window} /
 *    {@code WWcli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -DWWcli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -DWWcli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("WWcli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("WWcli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("WWcli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
