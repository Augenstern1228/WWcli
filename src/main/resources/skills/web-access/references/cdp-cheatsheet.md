...
WWcli 第 13 期接入 Google 官方 `chrome-devtools-mcp@latest`（28 个工具），第 14 期加 `/browser connect` 切 shared 模式。
...
- `new_page` — 开新 tab（WWcli 记录为 agent-opened，可被 close_page 关）
- `select_page` — 切换激活 tab
- `close_page` — 关 tab（**只能关 WWcli 自己 new_page 出来的**）
- `list_pages` — 列出所有 tab
