...
import com.WWcli.tui.config.TuiConfigPanel;
 ...
 *
 * <p>TUI 触发条件：
 * - {@code WWCLI_RENDERER=lanterna|tui} 环境变量或 {@code -DWWcli.renderer=lanterna|tui}
 * - 兼容旧入口：{@code WWCLI_TUI=true} 环境变量或 {@code -DWWcli.tui=true} 系统属性
 *
 * <p>即使显式启用，以下情况仍降级：
...
public final class TuiBootstrap {
    ...
    private static final String TUI_ENV = "WWCLI_TUI";
    private static final String TUI_PROPERTY = "WWcli.tui";
    private static final String RENDERER_ENV = "WWCLI_RENDERER";
    private static final String RENDERER_PROPERTY = "WWcli.renderer";
    ...
    public static boolean shouldUseTui(Terminal terminal) {
        ...
        if (Boolean.parseBoolean(Objects.requireNonNullElse(
            ...
            System.out.println(AnsiStyle.heading("💡 提示: NO_TUI=true，已切换为 CLI 模式。"
                    + "要启用 TUI 请清除 NO_TUI 环境变量，并使用 WWCLI_RENDERER=lanterna。"));
            return false;
        }
     ...
     */
    public static void launch(WWcliConfig config,
                              LlmClient llmClient,
                              Agent reactAgent,
