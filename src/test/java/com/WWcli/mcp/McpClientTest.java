...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("WWcli.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("WWcli.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("WWcli.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("WWcli.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不再发 shutdown notification（server 卡死时会让 WWcli 退出阻塞）。
