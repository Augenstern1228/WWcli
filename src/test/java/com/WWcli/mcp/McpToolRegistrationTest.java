...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("WWcli.audit.dir");
        System.setProperty("WWcli.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("WWcli.audit.dir");
            } else {
                System.setProperty("WWcli.audit.dir", previous);
            }
        }
    }
