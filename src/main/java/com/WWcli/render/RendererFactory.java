...
import com.WWcli.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -DWWcli.renderer} > {@code WWCLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code WWCLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        String prop = System.getProperty("WWcli.renderer");
        if (prop != null && !prop.isBlank()) {
            return parse(prop);
        }
        String env = System.getenv("WWCLI_RENDERER");
        if (env != null && !env.isBlank()) {
            return parse(env);
        }
        // 兼容旧 WWCLI_TUI=true → lanterna
        String legacyTui = System.getenv("WWCLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ WWCLI_TUI=true 已废弃，请改用 WWCLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 WWCLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
