package com.WWcli.agent;

import com.WWcli.llm.LlmClient;
import com.WWcli.llm.LlmTraceLogger;
import com.WWcli.context.ContextProfile;
import com.WWcli.context.TokenUsageFormatter;
import com.WWcli.lsp.LspDiagnosticReport;
import com.WWcli.memory.ConversationHistoryCompactor;
import com.WWcli.memory.ExplicitMemoryHints;
import com.WWcli.memory.MemoryManager;
import com.WWcli.prompt.PromptAssembler;
import com.WWcli.prompt.PromptContext;
import com.WWcli.prompt.PromptMode;
import com.WWcli.prompt.ProjectMemoryLoader;
import com.WWcli.render.PlainRenderer;
import com.WWcli.render.Renderer;
import com.WWcli.render.StatusInfo;
import com.WWcli.runtime.CancellationContext;
import com.WWcli.skill.SkillContextBuffer;
import com.WWcli.skill.SkillIndexFormatter;
import com.WWcli.skill.SkillRegistry;
import com.WWcli.util.AnsiStyle;
import com.WWcli.tool.ToolRegistry;
import com.WWcli.tool.ToolRegistry.ToolExecutionResult;
import com.WWcli.tool.ToolRegistry.ToolInvocation;
import com.WWcli.util.TerminalMarkdownRenderer;
import com.WWcli.image.ImageReferenceParser;
...
public class Agent {
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load WW.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.WWcli.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.WWcli.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.WWcli.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.WWcli.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.WWcli.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.WWcli.memory.MemoryEntry.estimateTokens(
